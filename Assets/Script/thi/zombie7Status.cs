...
public class normalZombie : MonoBehaviour
{
  ...
  void Update()
  {
    ...
    if (isRun)
    {
      ...
      if ((float)health <= 0)
      {
        ...
        Destroy(gameObject);
        if (gameManager.countToWin == 1)
        {
          gameManager.win();
        }
        else
        {
          gameManager.countToWin--;
        }
      }
    }
    else
    {
      ...
      if ((float)health <= (MaxHealth / 4))
      {
        zombieAnimator.SetInteger("status", 6);
      }
      if ((float)health <= 0)
      {
        gameManager.sound_die.Play();
  ...
  private void OnTriggerEnter2D(Collider2D collision)
  {
    ...
    {
      health = 0;
      GameObject MyAsh = Instantiate(ash, gameObject.transform.position, Quaternion.identity);
      Destroy(MyAsh, 1f);
    }
    ...
    if (collision.CompareTag("LawnMower"))
    {
      ...
      // Animation chết trước khi huỷ (bị lỗi end game luôn)
      //isRun = !isRun;
      //gameManager.sound_die.Play();
